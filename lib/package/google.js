        ...
        ? lang.list[0][langId]
            ? text
                ? await axios.get(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=${langId}&dt=t&q=${text}`)
                    ...
                    .catch(() => (function(){throw `${response.google.failGetData()} ${response.expression.sad()}`}()))
            : (function(){throw `${response.google.noText()} ${response.expression.angry()}`}())
        : (function(){throw `${response.google.unLang()} ${response.expression.angry()}`}())
    : (function(){throw `${response.google.noLang()} ${response.expression.angry()}`}()),
...
// module.exports = {
//     trs: async function(a, b){
//         switch(a){
//             case list0[a]:
//             return await axios.get(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=${a}&dt=t&q=${b}`).then(r => {return{from: langId[r.data[2]] ? langId[r.data[2]] : r.data[2], to: langId[a], raw: r.data[0].map(x => x[1]), translated: r.data[0].map(x => x[0])}}).catch(e => e)
//             break
//             default:
//             throw new Error('Kode bahasa kosong/Tidak terdaftar!')
//             break
//         }
//     },
//     tts: async function(a, b){
//         switch(a){
//             case list0[a]:
//             return `https://translate.google.com/translate_tts?ie=UTF-8&q=${b}&tl=${a}&total=1&idx=0&textlen=11&client=tw-ob&prev=input&ttsspeed=1`
//             break
//             default:
//             throw new Error('Kode bahasa kosong/Tidak terdaftar!')
//             break
//         }
//     },
//     trs_inline_mode: async function(b){
//         return Promise.all(list1.map(async(x) => await axios.get(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=${x}&dt=t&q=${b}`).then(r => {return{from: langId[r.data[2]] ? langId[r.data[2]] : r.data[2], to: langId[x], raw: r.data[0].map(x => x[1]), translated: r.data[0].map(x => x[0])}}).catch(e => e)))
//     },
//     tts_inline_mode: async function(b){
//         return list1.map((x) => ({id: langId[x], url: `https://translate.google.com/translate_tts?ie=UTF-8&q=${b.split(' ').join('+')}&tl=${x}&total=1&idx=0&textlen=11&client=tw-ob&prev=input&ttsspeed=1`}))
//     },
//     gnews: async function(a, b){
//         return a ? await axios.get(`https://news.google.com/rss/search?q=${a}&hl=${b}&gl=${b}&ceid=${b}`).then((r) => r.data).catch((e) => e) : await axios.get(`https://news.google.com/rss?hl=${b}&gl=${b}&ceid=${b}`).then((r) => r.data).catch((e) => e)
//     }
// }