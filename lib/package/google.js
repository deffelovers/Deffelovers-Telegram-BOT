...
module.exports = {
    ...
    // fungsi translate.
    trs: async (langId, text) => lang.list[0][langId]
        ? await axios.get(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=${langId}&dt=t&q=${text}`)
            ...
            .catch((function(){throw response.google.failGetData()}()))
    : (function(){throw response.google.noLang()}()),

    // fungsi speech.
    tts: (langId, text) => lang.list[0][langId]
        ? {speech: `https://translate.google.com/translate_tts?ie=UTF-8&q=${text}&tl=${langId}&total=1&idx=0&textlen=11&client=tw-ob&prev=input&ttsspeed=1`}
    : (function(){throw response.google.noLang()}())
}
