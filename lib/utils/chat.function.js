...
module.exports = {
    messageFunction: (ctx, bot, Downloaded) => {
        ...
        const call = {
            Basic: {
                start: () => typing() && ctx.reply(command.start(), reply_toMessage()),
            },
            Google: {
                isTranslate: () => typing() && ctx.reply(command.trs(), reply_toMessage()),
                isTextToSpeech: () => typing() && ctx.reply(command.tts(), reply_toMessage())
            }
        }
        
        return {
            isInvalid: (featureName) => typing() && ctx.reply(textReply.invalid.input, reply_markdown_withButton([[{text: textReply.button.needDocument, url: toBotBs64(`{type: 'dock', name: ${featureName}}`)}, {text: textReply.button.reportError, url: toBotBs64(`{type: 'report', name: ${featureName}}`)}]])),
            isFromButton: (input) => 
                console.log(toBotString(input))
                // .match(/^\/\w+/)?.[0] === '/trs'
                //     ? call.Google.isTranslate()
                // : toBotString(input).match(/^\/\w+/)?.[0] === '/tts'
                //     ? call.Google.isTextToSpeech()
                // : ''
                ,
            feature: {
                call: call,
                Google: { 
                    translate: (langId, text) => trs(langId, text)
                        .then((res) => typing() && ctx.reply(template.trs(res.from, res.to, res.translated), reply_toMessage_markdown()))
                        .catch((err) => typing() && ctx.reply(err, reply_toMessage())),
